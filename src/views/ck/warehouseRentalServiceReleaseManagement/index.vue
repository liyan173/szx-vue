      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="上架日期" style="width: 308px">
              <el-date-picker
                v-model="dateRange"
...
<!--        </el-row>-->
      ...
      <el-table v-loading="loading" border :data="warehouseRentApplyList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="广告位" align="center" prop="advertisingSpot">
          <template #default="scope">
            <el-tag :type="scope.row.advertisingSpot ? 'success' : 'info'">
              {{ scope.row.advertisingSpot ? '展示' : '不展示' }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="900px" append-to-body>
      <el-form ref="warehouseRentApplyFormRef" :model="form" :rules="rules" label-position="top">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="广告位" prop="advertisingSpot">
              <el-switch
                v-model="form.advertisingSpot"
                active-text="展示"
                inactive-text="不展示"
              ></el-switch>
            </el-form-item>
...
import { listWarehouse, WarehouseVO } from '@/api/ck/warehouse';
...
const getList = async () => {
  loading.value = true;
  const res = await listWarehouseRentApply(proxy?.addDateRange(queryParams.value, dateRange.value));
